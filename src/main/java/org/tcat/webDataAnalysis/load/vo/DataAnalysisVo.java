...
public class DataAnalysisVo {
    ...
    public Integer getPriority() {
        if (!StringTools.isNotEmptyByTrim(id) && StringTools.isNotEmptyByTrim(extendsInId)) {
            //有id无extends
            return DataAnalysisPriority.priorityFisrt.value();
        } else if (!StringTools.isNotEmptyByTrim(id) && !StringTools.isNotEmptyByTrim(extendsInId)) {
            //有extends有id
            return DataAnalysisPriority.prioritySecond.value();
        } else if (StringTools.isNotEmptyByTrim(id) && !StringTools.isNotEmptyByTrim(extendsInId)) {
            //有extends无id
            return DataAnalysisPriority.priorityThird.value();
        } else if (StringTools.isNotEmptyByTrim(id) && StringTools.isNotEmptyByTrim(extendsInId)) {
            //无extens无id
            return DataAnalysisPriority.priorityFourth.value();
        }
        return null;
    }
