...
public class WebDataDownload {
    ...
    public WebDataDownload() {
        List<WebDataVo> cDatas = new LoadXML().load(ClassLoader.getSystemResourceAsStream(XMLNAME), CHARSET);
    ...
    private void singleThread(WebDataVo cData, List<String> lineConcent) {
        for (String lc : lineConcent) {
            new SingleRunnable().download(lc, cData.getOutFile(), cData.getOutFileCharset(), getHttpMessageType(cData.getType()));
            try {
...
//        ThreadPoolExecutor pool = new ThreadPoolExecutor(5,20,200, TimeUnit.MILLISECONDS,new ArrayBlockingQueue<Runnable>(3), new ThreadPoolExecutor.CallerRunsPolicy());
        ...
        try {
            for (String lc : lineConcent) {
                Runnable r = new MultiRunnable(latch, lc, cData.getOutFile(), cData.getOutFileCharset(), getHttpMessageType(cData.getType()));
...
//      }
    ...
    private static String XMLNAME = "webDataDownload.xml";
